  ...
  useEffect(() => {
    if (apiData) {
      ...
      setChartData(
        transposeToChartDataForEdit(
          categories,
          apiData?.templateData?.scoreScale?.start
        )
      );
  ...
  const onChange = (
    ...
    setFormState((prevState) => {
      ...
      if (answer) {
        obj[id] =
          type === "score"
            ? { ...answer, score: newValue as number }
            : { ...answer, motivation: newValue as string };
      }
      return obj;
    });
    setChartData((prevState) =>
      prevState.map((elem) => {
        if (elem.id === id) {
          return type === "score"
            ? { ...elem, user1: newValue as number }
            : { ...elem, ["user1-motivation"]: newValue as string };
        } else {
          return elem;
        }
      })
  ...
  return (
    <div id="wrapper" className="flex flex-col text-xs pb-4">
      <Header
        ...
        titleSize="l"
        description={
          "Du har blivit inbjuden att ge feedback. Dina svar är anonyma. Du svarar genom att betygsätta "
        }
        hideLogin={true}
      />
      <div className="mb-4">
        <div className="flex justify-between">
          <span>
            <Label htmlFor="name">
              {nameIsMandatory === NameIsMandatory.NAMNGIVET
                ? "namn"
                : "Autogenererat namn"}
            </Label>
            <Input
              disabled={nameIsMandatory !== NameIsMandatory.NAMNGIVET}
              id="name"
              className="mt-1"
              value={userName}
              placeholder="Namn"
        ...
        {formHasError() && userName === "" && (
          <ErrorP text="Namn är obligatoriskt" />
        )}
      </div>
      <>
                  ...
                  {category.questions.map((question, ii) => (
                    <div key={ii} className="flex flex-col mb-8">
                      <div className="md:flex md:flex-row md:justify-between">
                        ...
                        <ScoreComponent
                          id={question.id}
                          start={apiData?.templateData.scoreScale.start ?? 0}
                          end={apiData?.templateData.scoreScale.end ?? 6}
                          formState={formState}
                      ...
                      <Input
                        ...
                        className="mt-2 mb-2"
                        placeholder="Varför gav du betyget?"
                        onChange={(e) =>
                          onChange(e.target.value, question.id, "text")
                        }
                      />
                      {shouldValidate &&
                        formState[question.id].motivation === "" && (
                          <ErrorP text="Fältet är obligatoriskt" />
                        )}
                    </div>
                  ))}
                </AccordionContent>
  ...
  return (
    <ResponsiveContainer width="99%" height="99%">
      <RadarChart
        ...
        outerRadius="80%"
        data={chartData}
        margin={{ top: 0, left: 65, right: 65, bottom: 0 }}
      >
        <defs>
          <radialGradient
        ...
        />
        <Radar
          name={dataIsAcc ? "Medelvärde" : "Ditt svar"}
          dataKey={"user1"}
          stroke="#dbeafe"
...
const ScoreComponent = ({
  ...
  formState: Answer;
  onScoreChange: (newScore: number, id: string, type: "score" | "text") => void;
  disabled: boolean;
}) => {
