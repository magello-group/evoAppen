...
export const initializeFormData = (templateData: TemplateData): Answer => {
  const allQuestions: Question[] = templateData.categories.flatMap(
...
export const transposeToChartDataForEdit = (
  categories: Category[],
  startScore: number
): ChartData[] => {
...
export const transposeToChartDataforView = (
...
export const transposeToAcculatedData = (
  chartData: ChartData[],
  users: string[]
) => {
  const categoryData: {
    [key: string]: { [key: string]: { sum: number; count: number } };
  } = {};
  ...
  const result: ChartData[] = Object.keys(categoryData).map((category) => {
    ...
    for (const user in categoryData[category]) {
      obj[user] = parseFloat(
        (
          categoryData[category][user].sum / categoryData[category][user].count
        ).toFixed(2)
      );
    }
    return obj;
